...
2. Creer ou selectionner un package ABAP, par exemple `ZSD_SALES_COCKPIT`.
3. Cloner le depot GitHub dans abapGit ADT.
4. Si le depot ne contient que les sources UI5/CAP, abapGit ne creera pas l'application BSP finale tout seul.
5. Deployer d'abord le contenu `dist/` en BSP dans le systeme ABAP.
6. Depuis le systeme ABAP, serialiser ensuite l'application BSP avec abapGit. Cette operation produira les objets ABAP/BSP attendus dans `src/`, notamment l'objet de type BSP application.
7. Pousser ces objets serialises dans GitHub.
8. Les prochains imports via abapGit ADT pourront alors recreer les objets ABAP/BSP dans un autre systeme.
...
Pour importer avec abapGit ADT, le depot doit contenir des objets ABAP serialises dans `src/`. Ces objets doivent etre generes depuis un systeme ABAP apres creation/deploiement du BSP.
